...
utils.check_dir(MODEL_DIR)

def parse_global_args(parser):
    """ 定义全局参数 """
    parser.add_argument('--gpu', type=str, default='0',
                        help='Set CUDA_VISIBLE_DEVICES, default 0 for GPU')
...
def main():
    ...
    logging.basicConfig(filename=args.log_file, level=args.verbose, format='%(asctime)s - %(message)s', force=True)
    ...
    if not args.regenerate and os.path.exists(corpus_path):
        ...
        logging.info('Generating corpus (First run may take time)...')
...
def save_rec_results(dataset, runner, topk):
    model_name_str = '{0}{1}'.format(init_args.model_name, init_args.model_mode)
    safe_dataset_name = dataset.corpus.dataset.replace('/', '_').replace('\\', '_')
    result_path = os.path.join(runner.log_path, 'rec-{}-{}.csv'.format(model_name_str, dataset.phase))
    ...
    logging.info("{} Prediction results saved to: {}".format(dataset.phase, result_path))

if __name__ == '__main__':
    init_parser = argparse.ArgumentParser()
    init_parser.add_argument('--model_name', type=str, default='MGDCF', help='Choose a model to run.')
    init_parser.add_argument('--model_mode', type=str, default='', help='Model mode suffix')
    ...
    if 'Context' in model_name.reader:
        args.data_appendix = '_context%d%d%d'%(args.include_item_features, args.include_user_features, args.include_situation_features)

    args.path = DATA_DIR

    safe_dataset_name = args.dataset.replace('/', '_').replace('\\', '_')
    log_args = [init_args.model_name + init_args.model_mode, safe_dataset_name + args.data_appendix, str(args.random_seed)]
    ...
    if args.log_file == '':
        args.log_file = os.path.join(LOG_DIR, init_args.model_name + init_args.model_mode, log_file_name + '.txt')
    if args.model_path == '':
        args.model_path = os.path.join(MODEL_DIR, init_args.model_name + init_args.model_mode, log_file_name + '.pt')
